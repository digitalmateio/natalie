		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			switch ($field_type->name) {
						...
						foreach ($default_val_arr as $uploadId) {
							...
							if(isset($upload->id)) {
								...
								if(in_array($upload->extension, ["jpg", "png", "gif", "jpeg"])) {
									$fileImage = "<img src='".current($upload->thumbnails)."'>";
								} else {
									$fileImage = "<i class='fa fa-file-o'></i>";
								}
								$uploadImages .= "<a class='uploaded_file2' upload_id='".$upload->id."' target='_blank' href='".$upload->url."'>".$fileImage."<i title='Remove File' class='fa fa-times'></i></a>";
							}
						}
				...
				case 'Image':
					...
					if(isset($upload->id)) {
						...
						$out .= "<a class='btn btn-default btn_upload_image hide' file_type='image' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
							<div class='uploaded_image'><img src='".$upload_src."'><i title='Remove Image' class='fa fa-times'></i></div>";
					} else {
				...
				case 'Multiselect':
					...
					$out .= Form::select($field_name."[]", $popup_vals, null, $params);

					if(is_array($default_val))
						foreach($default_val as $key => $value){
							$out = str_replace('value="'.$value.'"', 'value="'.$value.'" selected="selected"', $out);
						}
					break;
				case 'Name':
					$out .= '<label for="'.$field_name.'">'.$label.$required_ast.' :</label>';
