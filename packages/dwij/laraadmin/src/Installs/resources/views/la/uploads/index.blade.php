...
$(function () {
    ...
    $("body").on("click", "ul.files_container .fm_file_sel", function() {
        ...
        $("#EditFileModal #downFileBtn").attr("href", upload.url+"?download");
        ...
        if($.inArray(upload.extension, ["jpg", "jpeg", "png", "gif", "bmp"]) > -1) {
            $("#EditFileModal .fileObject").append('<img src="'+upload.url+'">');
            $("#EditFileModal .fileObject").css("padding", "15px 0px");
        } else {
            switch (upload.extension) {
                case "pdf":
                    // TODO: Object PDF
                    $("#EditFileModal .fileObject").append('<object width="100%" height="325" data="'+upload.url+'"></object>');
...
function formatFile(upload) {
    var image = '';
    if($.inArray(upload.extension, ["jpg", "jpeg", "png", "gif", "bmp"]) > -1) {
        image = '<img src="'+upload.thumbnails[Object.keys(upload.thumbnails)[0]]+'">';
    } else {
        switch (upload.extension) {
            case "pdf":
