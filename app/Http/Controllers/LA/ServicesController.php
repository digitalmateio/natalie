...
class ServicesController extends Controller
{
	...
	public $listing_cols = ['id', 'parent_id', 'title_ge', 'title_en', 'title_ru', 'price'];
	
	public function __construct() {
		// Field Access of Listing Columns
		if(\Dwij\Laraadmin\Helpers\LAHelper::laravel_ver() >= 5.3) {
			$this->middleware(function ($request, $next) {
				$this->listing_cols = ModuleFields::listingColumnAccessScan('Services', $this->listing_cols);
				return $next($request);
			});
		} else {
			$this->listing_cols = ModuleFields::listingColumnAccessScan('Services', $this->listing_cols);
	...
	{
		$values = DB::table('services')->select($this->listing_cols)->whereNull('deleted_at');
